...
import { CATEGORIES as DEFAULT_CATEGORIES } from "./categories";
...
async function doSeed(): Promise<void> {
  ...
  const categories = await col<CategoryRecord>("categories");
  ...
  // Kategori jasa disimpan di koleksi terpisah supaya bisa
  // ditambah/dihapus dari dashboard admin tanpa deploy ulang. Di-seed dari
  // daftar bawaan (src/lib/categories.ts) hanya sekali, saat koleksinya
  // masih kosong.
  ...
  if (existingCategories === 0) {
    ...
    try {
      await categories.insertMany(
        DEFAULT_CATEGORIES.map((name) => ({ id: newId("cat"), name, createdAt: seedNow })),
        { ordered: false }
      );
    } catch (err) {
...
export const store = {
  ...
  /** Dipakai dashboard admin: semua profil apa pun statusnya, bukan hanya yang tayang publik. */
  async adminListProfiles(): Promise<ProfessionalProfileRecord[]> {
    await ensureSeeded();
    const profiles = await col<ProfessionalProfileRecord>("profiles");
    return profiles.find({}).sort({ category: 1 }).toArray();
  },
  ...
  async listCategories(): Promise<CategoryRecord[]> {
    await ensureSeeded();
    const categories = await col<CategoryRecord>("categories");
    return categories.find({}).sort({ name: 1 }).toArray();
  },
  async createCategory(name: string): Promise<CategoryRecord> {
    await ensureSeeded();
    const categories = await col<CategoryRecord>("categories");
    const dup = await categories.findOne({
      name: { $regex: `^${name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, $options: "i" },
    });
    if (dup) throw new Error("Kategori dengan nama itu sudah ada");
    const record: CategoryRecord = { id: newId("cat"), name, createdAt: new Date().toISOString() };
    await categories.insertOne(record);
    return record;
  },
